...
import KradSiteRadiator, {
  KradSiteRadiatorType,
} from './models/kradSiteRadiator.model';
...
const updateSiteDB = async (_req: Request, res: Response) => {
  const generalCsv: KradSiteCsvModel[] = (await googleSheetRead(
    GENERAL_DOC_ID
  )) as KradSiteCsvModel[];
  ...
  const generalData: KradSiteModelType[] = generalCsv.map(general => {
    return {
      ...
      conn_bottom_right: general.conn_bottom_right
        ? +general.conn_bottom_right
        : null,
      conn_bottom_left: general.conn_bottom_left
        ? +general.conn_bottom_left
        : null,
      conn_bottom_center: general.conn_bottom_center
        ? +general.conn_bottom_center
        : null,
      conn_bottom_double_sided: general.conn_bottom_double_sided
        ? +general.conn_bottom_double_sided
        : null,
  ...
  for (const model of generalData) {
    ...
    const csvRadiators = (await googleSheetRead(
      `d/${model.doc_id}/`
    )) as KradSiteCsvRadiator[];
