...
import KradModel, { KradModelType } from './models/kradModel.model'
...
const get = async (req: Request, res: Response): Promise<void> => {
	try {
		...
		const [connectionCode, connection] = Object.entries(connectionNames).find(
			([code, value]) => value.urlParam === connectionUrlParam
		) || [null, null]
		const [montageCode, montage] = Object.entries(montageNames).find(
			([code, value]) => value.urlParam === montageUrlParam
		) || [null, null]
