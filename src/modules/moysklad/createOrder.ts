...
const createOrder = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!version) {
    res.status(400).send('cant find version');
    return;
  }
  const project = await Project.findOne({ _id: version.project });
  if (!project) {
    res.status(400).send('cant find project');
    return;
  }

  let name = await getUniqName('customerorder', project.title, 0);

  const rooms = await Room.find({ version: versionId })
    .populate(['itemAccessories', 'itemRadiators'])
    .lean();
  if (!rooms || rooms.length === 0) {
    res.status(400).send('no rooms');
    return;
  }
  // @ts-ignore
      ...
      if (newItemName !== null) {
        ...
        if (newProd) {
          ...
          await MoyskladItemHref.findOneAndUpdate(
            {
              accessory: roomItem.accessory,
              model: roomItem.model,
              color: roomItem.color,
              connection: roomItem.connection,
              sections: roomItem.sections,
            },
  ...
  const result = await moyskladHttp.post('/entity/customerorder', {
    ...
    positions: items
      .sort((a, b) =>
        a.accessory == undefined && b.accessory !== undefined ? -1 : 1
      )
      .map(item => {
        return {
          ...
          assortment: {
            meta: {
              type: 'product',
              href: item.metaHref.replace('//online', '//api'),
            },
          },
        };
      }),
