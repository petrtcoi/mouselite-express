...
const getRoomItemPriceX100 = async (props: Props): Promise<number> => {
    ...
    const currencyRate = dbItem.currency === 'rub' ? 1 :
        dbItem.currency === 'eur' ? props.currencies.eur :
            dbItem.currency === 'varmann' ? props.currencies.varmann : 0
    ...
    if (props.item.connection) {
        ...
        if (!!dbConnection) {
            const connCurrencyRate = dbConnection.currency === 'rub' ? 1 :
                dbConnection.currency === 'eur' ? props.currencies.eur :
                    dbConnection.currency === 'varmann' ? props.currencies.varmann : 0
    ...
    return Math.floor((price + connectionPrice) * (100 - (props.item.discount || 0)) / 100) * 100
}
