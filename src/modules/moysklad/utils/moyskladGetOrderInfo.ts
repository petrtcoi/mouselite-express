...
const moyskladGetOrderInfo = async ({
  orderName,
}: {
  orderName: string;
}): Promise<{ order?: MoyskaldOrder; error?: boolean }> => {
  // console.log(`/entity/customerorder?filter=name=${orderName}`)
  const result =
    (await moyskladHttp.get(
      `/entity/customerorder?filter=name=${orderName}`
    )) || undefined;
  ...
  const shippedSum = order.rows[0].shippedSum
    ? order.rows[0].shippedSum / 100
    : 0;
  const agent = order.rows[0].agent;
  const organization = order.rows[0].organization;

  const positions: any = await moyskladHttp.get(
    order.rows[0].positions.meta.href
  );
  const productRows = positions.data.rows;

  const items: MoyskaldItem[] = await productRows.reduce(
    async (acc: MoyskaldItem[], row: any) => {
      ...
      const newItem: MoyskaldItem = {
        ...
        shipped: row.shipped,
        paymentObject:
          res.data.paymentItemType === 'SERVICE' ? 'service' : 'commodity',
      };
      return [...data, newItem];
    },
    []
