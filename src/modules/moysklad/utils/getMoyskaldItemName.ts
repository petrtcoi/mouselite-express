...
const getMoyskaldItemName = async (props: Props): Promise<string | null> => {
	...
	if (props.item.accessory) {
		...
		// Мощность
		const dt50 = model.dt50
			? props.item.sections
				? Math.round(model.dt50 * props.item.sections)
				: model.dt50
			: undefined
		const dt60 = model.dt60
			? props.item.sections
				? Math.round(model.dt60 * props.item.sections)
				: model.dt60
			: undefined
		const dt70 = model.dt70
			? props.item.sections
				? Math.round(model.dt70 * props.item.sections)
				: model.dt70
			: undefined
