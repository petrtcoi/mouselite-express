...
import { TubogCsvModelType } from '../tubogshop/typescript/types/tubgoCsvModel.type'

const MIN_SECTIONS = 4

export const seedTubog = async (
	_req: Request,
	res: Response
): Promise<void> => {
	const GOOGLE_DOC_ID_MODELS =
		'd/1K6PFmnsAklsE-8jORAIhffg8B542FNBjJNKJQ2pwomY/e'
	const GOOGLE_DOC_ID_CONNECTIONS =
		'd/1pABKLGYdkAwk56OfK-m6zll7N14hiYf2LlBS4zmZh98/e'

	const csvModels = (await googleSheetRead(
		GOOGLE_DOC_ID_MODELS
	)) as TubogCsvModelType[]

	let modelsCount = 0

	// for (const csv of csvModels) {
	// 	const [min, max] = csv.inStockSections
	// 		? csv.inStockSections.split('-').map(x => +x)
	// 		: [0, -1]
	...
	// 		height: +csv.height,
	// 		lengthSection:
	// 			csv.lengthSection && +csv.lengthSection > 0 ? +csv.lengthSection : 0,
	...
	// 		priceSection:
	// 			csv.priceSection && +csv.priceSection > 0 ? +csv.priceSection : 0,
	// 		imageMain: csv.imageMain.trim(),
	// 		imagesList:
	// 			csv.imagesList && csv.imagesList.length > 0
	// 				? csv.imagesList.split(',').map(img => img.trim())
	// 				: [],
	...
	res.status(200).send({ csvModels: csvModels.length, example: csvModels[10] })
}
