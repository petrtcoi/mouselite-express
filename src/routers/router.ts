...
router.get('/user/checktoken',
    authAllUsers,
    userFuncs.checkToken
);
router.get('/user',
    authAllUsers,
    userFuncs.getList
...
router.get('/twilio',
    authAllUsers,
    twilioFuncs.getMsgList
);


router.patch('/product',
    authAllUsers,
    productFuncs.productUpdateAll
);
router.get('/product',
    authAllUsers,
    productFuncs.get
);
router.get('/product/:projectid',
    authAllUsers,
    productFuncs.get
);
router.post('/product/',
...
router.get('/itemradiator/:id',
    authAllUsers,
    itemRadiatorFuncs.get
);
router.post('/itemradiator',
    authAllUsers,
    checkPresence(['roomId', 'modelId']),
    itemRadiatorFuncs.itemRadiartorCreate
);
router.patch('/itemradiator',
    authAllUsers,
    checkPresence(['id', 'updates']),
    itemRadiatorFuncs.itemRadiartorUpdate
);
router.delete('/itemradiator',
    authAllUsers,
    checkPresence(['id']),
    itemRadiatorFuncs.itemRadiartorDelete
);

router.get('/itemaccessory/:id',
    authAllUsers,
    itemAccessoryFuncs.get
);
router.post('/itemaccessory',
    authAllUsers,
    checkPresence(['roomId', 'accessoryId']),
    itemAccessoryFuncs.itemAccessoryCreate
);
router.patch('/itemaccessory',
    authAllUsers,
    checkPresence(['id', 'updates']),
    itemAccessoryFuncs.itemAccessoryUpdate
);
router.delete('/itemaccessory',
    authAllUsers,
    checkPresence(['id']),
...
router.get('/offerlist/:projectId',
    offerFuncs.getList
);


router.post('/ozon/oventrop',
    ozonFuncs.updateStockOventrop
);
router.post('/ozon/arbonia',
    ozonFuncs.updateStockArbonia
);



router.post('/kkt/checkout',
    authAllUsers,
    checkPresence(['orderName', 'client', 'receipt']),
    kktFuncs.checkout
);
router.get('/kkt/order/:orderName',
    authAllUsers,
    kktFuncs.getOrder
);

router.post('/email',
    emailFuncs.sendEmail
);
...
router.post('/krad/site',
    authAllUsers,
    kradFuncs.updateSiteDb
);
router.get('/krad',
    authAllUsers,
    kradFuncs.get
);
router.get('/krad/site/:model',
    kradFuncs.getSiteModel
);

router.post('/moysklad/order',
    authAllUsers,
    checkPresence(['versionId']),
    moyskladFunc.createOrder
);
router.post('/moysklad/helper',
    authAllUsers,
    moyskladFunc.helper
);

router.post('/webasyst/moysklad/order',
    authAllUsers,
    checkPresence(['orderName', 'waStore']),
    webasystFunc.createMoyskladOrder
);

router.post('/tubog/db',
    authAllUsers,
    tubogFunc.updateDb
);
router.get('/tubog/model/:model',
    tubogFunc.getModel
);
router.get('/tubog/connection/:connection',
    tubogFunc.getConnection
);
router.get('/tubog/model/',
    tubogFunc.getAllModels
);
router.get('/tubog/connection/',
    tubogFunc.getAllConnections
);

router.post('/omoikiri/csvparse',
    authAllUsers,
    omoikiriFunc.omoikiriParseGoogle
);
router.get('/omoikiri/skus',
    authAllUsers,
    omoikiriFunc.getAllSkus
);
router.get('/omoikiri/sku/:sku',
    omoikiriFunc.getBySku
);
